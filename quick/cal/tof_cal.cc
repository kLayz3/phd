...
int main(int argc, char* argv[]) {
	...
	for(const auto& fileName : f) {
        ...
        const double beta_inc = phy::Beta(A, Z, phy::Brho_t{run_info.brho.s1_s2});
        const double beta_out = phy::Beta(A, Z, phy::Brho_t{run_info.brho.s2_s3});
		WARN("[%s] calculated beta(S1-S2): %.4f ; beta(S2-S3): %.4f\n", fileName.c_str(), beta_inc, beta_out);
