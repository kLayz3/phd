...
void tpc_alignment(std::string prefix = "", std::vector<std::string> fileNames = {}, 
	int i_tpc=0,
	BinInfo bx = { .b = {.nBins = 1000, .lo = -100, .hi = 100}, .nBinsFit = 40 },
	BinInfo by = { .b = {.nBins = 600, .lo = -60, .hi = 60}, .nBinsFit = 40 }
) {
	...
	TH1I *h1_x[2], *h1_y[4];
	for(int i=0; i<2; ++i)
		h1_x[i] = new TH1I(Form("h1_TPC%d_x%d", i_tpc, i), Form("h1_TPC%d_x%d", i_tpc, i), bx.b.nBins, bx.b.lo, bx.b.hi);

	for(int i=0; i<4; ++i)
		h1_y[i] = new TH1I(Form("h1_TPC%d_y%d", i_tpc, i), Form("h1_TPC%d_y%d", i_tpc, i), by.b.nBins, by.b.lo, by.b.hi);
	...
	for(const auto& fileName: fileNames) {
		...
		for(int i=0; i<2; ++i) {
			...
			double fitMin = h->GetBinCenter(maxBin - bx.nBinsFit);
			double fitMax = h->GetBinCenter(maxBin + bx.nBinsFit);
		...
		for(int i=0; i<4; ++i) {
			auto* h = h1_y[i];
			int maxBin = h->GetMaximumBin();
			double fitMin = h->GetBinCenter(maxBin - by.nBinsFit);
			double fitMax = h->GetBinCenter(maxBin + by.nBinsFit);
			h->Fit("gaus", "Q", "", fitMin, fitMax);
			TF1* fitF = h->GetFunction("gaus");
	...
	json& j = jfull.at("TPC").at( label[i_tpc] );
	for(int i=0; i<2; ++i) {
