...
namespace util {
	...
	template <typename T>
	struct has_setname<T, std::void_t<decltype(std::declval<const T&>().SetName())>> : std::true_type {};
}
...
public:
            ...
            TOnceBase(name) {
                if constexpr(std::is_constructible_v<U, std::initializer_list<typename U::value_type>>)
					...
					if(il.size() == 0)
						...
						assert(il.size() == _internal.size() && "Initializer list for std::array<T,N> must have either 1 or N members"); 
				}
    ...
    Int_t Write(const char* name = "") override {
		if(!name) ERROR("Don't pass nullptr here.");
        if constexpr(std::is_base_of_v<TObject, T>) {
            return _internal.Write( *name ? name : _name.c_str());
        } else {
            TFile* outFile = gDirectory->GetFile();
            if(!outFile || !outFile->IsOpen())
                ERROR("%s (label: \'%s\') : output ROOT file not found or invalid.", _name.c_str(), name);
            return outFile->WriteObject(&_internal, *name ? name : _name.c_str());
        }
    }

    void SetName(std::string name, const char* title = "") {
        TOnceBase::SetName(name);
        if constexpr(std::is_base_of_v<TNamed, T> || util::has_setname<T>::operator bool) {
	...
	void* Load(TFile* f = nullptr, const char* target = "") override {
		...
		if(!name || ! *name) ERROR("Unnamed TOnce container while trying to load from a file.");
		if(!f) {
			f = gDirectory->GetFile();
			if(!f) ERROR("(%s) tried to open the file, but didn't receive a handle and also `gDirectory->GetFile()` call returned nullptr", _name.c_str());
			return this->Load(f, target);
		}
		T* tmp = f->Get<T>(name);
