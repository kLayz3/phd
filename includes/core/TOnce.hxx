...
public:
	...
	/* Case 5: Most general ctor with custom collector. Delegates to either [1] or [2] */
	template<typename... Ts>
		TOnce(void (*fn)(T&, const T&), Ts&&... args) : TOnce(std::forward<Ts>(args)...) 
		{ _collector = fn; }

	/* Case 5.5: same as previous, but with init list. */
	template<typename U = T, typename std::enable_if<util::has_value_type<U>::value>::type* = nullptr>
		TOnce(void (*fn)(T&, const T&), 
			const char* name, std::initializer_list<typename U::value_type> il) 
...
template<typename T>
std::unique_ptr<TOnceBase> TOnce<T>::Clone() const {
	...
	if constexpr(std::is_base_of_v<TH1, T>) {
		...
		if constexpr(util::has_copy<T>::value)
			...
			copy = std::make_unique<TOnce<T>> (this->_collector, this->GetName(),
...
auto Add(T& lhs, const T& rhs) -> decltype((void)(lhs += rhs)) {
	lhs += rhs; // Only valid if += operator is valid.
}
template<typename T>
auto Mean(T& lhs, const T& rhs) -> decltype((void)(lhs += rhs, lhs /= 2)) {
	lhs += rhs; // Only valid if +=operator(const T&) is valid.
	lhs /= 2;   // Only valid if /=operator(int) is valid.
}

template<typename T>
void TOnce<T>::Collect(const TOnceBase& rhs) {
	...
	if constexpr(util::has_dyadic_add_ref<T>::value) {
		...
		ERROR("(%s) - Name: '\%s\' ; Underlying type \'%s\' doesn't define how to add or mean-up two values, "
			...
			"Define a `void Add(T&, const T& )` function or pass a lambda as first argument "
