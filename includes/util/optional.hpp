...
// Edit by Martin B. 11/03/2026
// Adding the wrapper to mnd:: namespace
// plus an unwrap() method and ability to generate
// CERN ROOT dictionary as a build option.
...
# endif
  ...
  T unwrap() const {
	  if(!*this) throw std::runtime_error("optional<T>::unwrap() const, null value detected.");
	  return **this; 
  }
  T unwrap() {
	  if(!*this) throw std::runtime_error("optional<T>::unwrap(), null value detected.");
	  return constexpr_move(const_cast<optional<T>&>(*this).contained_val()); 
  }
};
...
{
  template <typename T>
  struct hash<mnd::optional<T>>
  {
    typedef typename hash<T>::result_type result_type;
    typedef mnd::optional<T> argument_type;
