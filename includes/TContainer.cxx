...
void TContainer::Setup(ContainerIO io_mode, TFile* f, TTree* t) {
	...
	if(!f || f->IsZombie() || !f->IsOpen()) {
		...
		switch(io_mode) {
			...
			/* This is fine. Since if we fetch the object in this call,
...
Int_t TContainer::Write() {
	if(_io_mode == ContainerIO::kINPUT)
		ERROR("Bad call to `TContainer::Write()` with its mode being in \'input\'. Name: " EMPH(%s), _name.c_str());
	if(!_file_p || _file_p->IsZombie() || !_file_p->IsOpen())
		ERROR("TFile* handle which isn't valid or isn't open. Was the TContainer::Setup(...) call successful? Name: " EMPH(%s), _name.c_str());
	return 0;
}
