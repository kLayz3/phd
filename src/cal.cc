...
int main(int argc, char *argv[]) {
    ...
    add_logged_option(app, "-s,--setup", setupFile, "Specify FRS JSON setup file name.")
        ->check(CLI::ExistingFile);
    add_logged_option(app, "-p,--foot-setup", footSetupFile, "Specify FOOT JSON setup file name.")
        ->check(CLI::ExistingFile);
    ...
    for(int i = 0; i < N_FOOT; ++i) {
        cfoot[i].Init( {{"ID"s, std::to_string(mfoot[i].FOOT_N)}, {"Setup"s, footSetupFile}} );
        cfoot[i].Setup();
    }
